...
function exiter($loc){exit(header("Location: $loc"));}

function handle_name($name){
	$name=trim($name);$sln=strlen($name);
...
function handle_email($email){
	$email=trim($email);$slem=strlen($email);
	if($slem<8||$slem>48){$email="E-mail expected to be min 12 et max 48 chars long";}
	elseif(!ctype_alnum(str_replace(array('_','-','.','@'),'',$email))){$email="E-mail expected to include only letters, numbers, underscore, hyphen, dot and @";}
	return $email;
}
...
function getdata($conn,$query,$prefix){
	extract(mysqli_fetch_assoc(sql_query($conn,$query)),EXTR_PREFIX_ALL,$prefix);
}
