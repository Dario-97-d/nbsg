...
<?php
$getpms=sql_query($conn,"SELECT m.*,u.id FROM mailbox m LEFT JOIN user u ON m.pmfrom=u.name WHERE id=$uid AND seen<>2");
if(mysqli_num_rows($getpms)<1){echo "No sent pms to show";}
else{
	while($pms=mysqli_fetch_assoc($getpms)){
		echo "<b>".date("d/m H:i:s",$pms['time'])."</b> || ";
		echo '<b>PM to:</b> <a href="nin?id='.$pms['id'].'">'.$pms['pmto']."</a> ";
		echo '<b>'.($pms['seen']==0?'Not s':'S').'een</b>';
		?>
		 || <a href="sendpm?to=<?php echo $pms['pmfrom'];?>">Send PM</a>
		<textarea name="pmtext" disabled><?php echo nl2br($pms['pmtext']);?></textarea>
		<br />
		<form action="pmsent" method="POST"><input type="submit" name="<?php echo $pms['pmid'];?>" value="Delete"/></form>
		<?php
	}
}
include("footer.php");?>