...
extract(sql_mfa($conn,"SELECT level,style,rank,name,nin1,nin2 FROM atts a JOIN clan c ON a.id=c.id JOIN user u ON c.id=u.id JOIN team t ON u.id=t.id WHERE u.id=$uid"));?>
<h1>Team</h1>
...
<table align="center" style="text-align:center;" cellpadding="8" cellspacing="0">
	<form method="POST">
		<?php echo "<tr><th>$style</th><td>$name</td><td>Lv $level</td></tr>";
		if($nin1>0||$nin2>0){
...
<?php if($nin1>0&&$nin2>0){?>
	<h3><a href="teamtrain">Team Train</a></h3>
	Go on Team Exam
	<br />
	(chunnin shiken)
	<h3>Rank-D</h3>
<table align="center" style="text-align:center;" cellpadding="8" cellspacing="0">
	<form method="POST">
		<?php $picks=sql_query($conn,"SELECT u.id,name,level,style FROM user u JOIN atts a ON u.id=a.id JOIN clan c ON u.id=c.id WHERE rank='D' AND level<=$level AND u.id NOT IN($uid,$nin1,$nin2) ORDER BY u.id DESC LIMIT 25");
...
<?php }else{?>
	...
	<h3><a href="teampick">List of nin</a></h3>
<?php }
include("footer.php");?>