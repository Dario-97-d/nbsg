...
extract( sql_mfa(
	$conn,
	"SELECT nin1, nin2, joint, name, rank, style, tskl
	FROM team t
	JOIN user u ON t.id = u.id
	JOIN clan c ON u.id = c.id
	JOIN styl s ON c.id = s.id
	WHERE u.id = $uid" ) );
...
$k = (substr(microtime(true), -1) % 10) + 3;
$s = (substr(microtime(true), -2, 1) % 10) + 3;
$t = (substr(microtime(true), -3, 1) % 10) + 3;
$n = (substr(microtime(true), -4, 1) % 10) + 3;
...
<table class="table-team" align="center" style="text-align: center;">
	...
	<tr>
		<td colspan="2">
			<div id="ttd" style="width: <?= round( $ken * $ratio ) ?>px; float: right"></div>
		</td>
	...
	<tr>
		<td colspan="2">
			<div id="ttd" style="width: <?= round( $shu * $ratio ) ?>px; float: right"></div>
		</td>
	...
	<tr>
		<td colspan="2">
			<div id="ttd" style="width: <?= round( $tai * $ratio ) ?>px; float: right"></div>
		</td>
	...
	<tr>
		<td colspan="2">
			<div id="ttd" style="width: <?= round( $nin * $ratio ) ?>px; float: right"></div>
		</td>
	...
	<tr>
		<td colspan="2">
			<div id="ttd" style="width: <?= round( $gen * $ratio ) ?>px; float: right"></div>
		</td>
...
$up_pow = round( 9 * $ken / $total );
$up_jut = round( 9 * $shu / $total );
$up_agi = round( 9 * $tai / $total );
$up_fla = round( 9 * $nin / $total );
$up_tac = round( 9 * $gen / $total );

$train = 10 - ( $up_fla + $up_pow + $up_agi + $up_jut + $up_tac );

switch ( $style )
{
	case 'Kensou': $up_pow += $train; break;
	case 'Surike': $up_jut += $train; break;
	case 'Geniru': $up_tac += $train; break;
